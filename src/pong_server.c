...
int open_udp_socket(int *pong_port)
{
	...
	for (port_number = IANAMINEPHEM; port_number <= IANAMAXEPHEM; ++port_number) {
		...
		if (gai_rv != 0) { //NULL -> si lega a tutte le interfacce di rete disponibili
		...
		if(udp_socket < 0) {
			fail_errno("UDP Pong not get socket\n");
		}
        // la bind associa il socket ad un indirizzo e una porta
		bind_rv = bind(udp_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen);
        if (bind_rv == 0) {
            *pong_port = port_number;
    	    freeaddrinfo(pong_addrinfo);
            return udp_socket; 
        } //avremmo dovuto controllare anche il caso in cui la bind fallisce
		/*** TO BE DONE END ***/
...
int main(int argc, char **argv)
{
	...
	if (gai_rv != 0) { //NULL -> si lega a tutte le interfacce di rete disponibili
		fail("Error while using getaddrinfo\n"); //avremmo dovuto usare fail_errno
	}
	
	server_socket = socket(server_addrinfo->ai_family, server_addrinfo->ai_socktype, server_addrinfo->ai_protocol);
	if(server_addrinfo < 0) { //CI ANDAVA SERVER_SOCKET
		fail_errno("Server could not get socket\n");
	}

	if(bind(server_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) {
		if (errno != EADDRINUSE) //indirizzo già in uso, SOLO FAIL_ERRNO
			fail_errno("Server could not bind the socket");
		if (close(server_socket))
			fail_errno("Server could not close the socket");
	}
    //SOMAXCONN num max di connessioni gestibili dal sis op
	if(listen(server_socket, SOMAXCONN) < 0) {
		fail_errno("Server could not start listening");
	}
	/*** TO BE DONE END ***/
