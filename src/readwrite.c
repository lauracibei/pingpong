...
ssize_t nonblocking_write_all(int fd, const void *ptr, size_t n)
{
	...
	while (n_left > 0) {
		...
		if (n_written < 0) {
			/*** TO BE DONE START ***/
			if (errno == EAGAIN || errno == EWOULDBLOCK) { //INDICANO CHE IL FD NON È PRONTO PER LA SCRITTURA
				// Set up the file descriptor set.
				fd_set writefds; //SI PREPARA UN SET DI FD USATO DALLA SELECT PER MONITORARE QUANDO UN FD È PRONTO
				FD_ZERO(&writefds); //INIZIALIZZAZIONE
				FD_SET(fd, &writefds); //AGGIUNGE IL FD AL SET DI SCRITTURA

				// MONITORA LO STATO DEL FD, ATTENDE CHE DIVENTI SCRIVIBILE
				int res = select(fd + 1, NULL, &writefds, NULL, NULL);
				if (res <= 0) { //ERRRORE 
					return -1;
				}
				// Try writing again after select indicates fd is ready.
				continue;
			}
			/*** TO BE DONE END ***/
